...
SERPER_KEY = "d4d300f619ad274ee3978e1000a09cb4be996b94"
...
# ---------------------------------
# Parsing sections (same logic you had)
# ---------------------------------
def parse_taboola_description(raw_html):
    decoded = html.unescape(raw_html or "")
    soup = BeautifulSoup(decoded, "html.parser")
    text = soup.get_text(separator="\n", strip=True)

    section_map = {
        "About Us": ["about taboola", "who we are", "our company"],
        "About the Role": ["we are looking for", "about the role", "role overview", "job summary", "position overview"],
        "What You'll Be Doing": ["what you'll be doing", "how you’ll make an impact", "responsibilities", "what you will do"],
        "Qualifications": ["requirements", "what you bring", "what we are looking for", "skills", "to thrive in this role", "must have"],
        "Life at": ["why taboola", "why you'll love", "perks", "culture", "life at taboola"],
        "In the News": ["in the news", "press", "media coverage"]
    }
...
def scrape_taboola_jobs(keyword="Data Scientist"):
    url = "https://boards-api.greenhouse.io/v1/boards/taboola/jobs"
    params = {"content": "true"}
    ...
    for job in data.get("jobs", []):
        ...
        if keyword.lower() in title.lower():
            parsed_desc = parse_taboola_description(job.get("content", ""))
            ...
            row = {
                ...
                "source": "Taboola Careers",
                "scraped_at": scraped_time,
                **parsed_desc
            }
            jobs.append(ensure_schema(row))

    print(f"Found {len(jobs)} Positions 'Data Scientist' at Taboola")
    return jobs
...
def upsert_and_mark_stale(path: str, new_rows: list):
    ...
    print(f"V Upsert complete. Wrote {len(merged)} rows to {path} (new={new_count}, stale={stale_count})")

if __name__ == "__main__":
    out_path = "taboola_parsed.csv"
    jobs = scrape_taboola_jobs(keyword="Data Scientist")
...
def parse_greenhouse_to_riskified_format2(raw_html):
    ...
    matches.sort()
    matches.append((len(text), None))  # סוף הטקסט
...
def scrape_similarweb_jobs(keyword="Data Scientist"):
    ...
    print(f"Found {len(jobs)} Positions 'Data Scientist' at Similarweb")
    return jobs
...
if __name__ == "__main__":
    out_path = "similarweb_parsed.csv"
...
def pick_date(job: dict) -> str:
    """
    Greenhouse: בד"כ יש updated_at; fallback ל-created_at.
    מחזיר ISO-8601 ב-UTC אם ניתן.
    """
    raw = (job.get("updated_at") or job.get("created_at") or "").strip()
    if not raw:
        return ""
...
# -----------------------------
# Section parser (your logic)
# -----------------------------
def parse_greenhouse_to_riskified_format1(raw_html):
    decoded = html.unescape(raw_html or "")
    soup = BeautifulSoup(decoded, "html.parser")
    text = soup.get_text(separator="\n", strip=True)

    section_map = {
        "About Us": ["about melio", "about us", "who we are", "our company", "melio is"],
        "About the Role": ["we are looking for", "we’re looking for", "about the role", "role overview", "why is this role", "in this position", "your mission", r"as a"],
        "What You'll Be Doing": ["key responsibilities", "what you'll be doing", "your role", "day-to-day", "so, what will you be doing", "what you will be doing", "a day in a life"],
        "Qualifications": ["requirements", "qualifications", "ideal candidate", "must have", "needed", "this is the perfect job"],
        "Life at": ["why melio", "why you’ll love", "why you'll love", "benefits", "perks", "life at melio", "you’ll find a home", "diversity isn’t just"],
        "In the News": ["in the news"]
    }
...
def scrape_melio_jobs(keyword="Data"):
    ...
    for job in data.get("jobs", []):
        title = (job.get("title") or "")
        if keyword.lower() in title.lower():
            parsed_desc = parse_greenhouse_to_riskified_format1(job.get("content", ""))
            ...
            row = {
                ...
                "date_posted": pick_date(job),
                "link": job.get("absolute_url"),
                "source": "Melio Careers",
                "scraped_at": scraped_time,
                **parsed_desc
            }
            jobs.append(ensure_schema(row))

    print(f"Found {len(jobs)} '{keyword}' positions at Melio")
    return jobs
...
if __name__ == "__main__":
    out_path = "melio_parsed.csv"
    jobs = scrape_melio_jobs(keyword="Data")
    upsert_and_mark_stale(out_path, jobs)
...
def pick_date(job: dict) -> str:
    """Riskified/Greenhouse: לרוב updated_at; fallback ל-created_at. מחזיר ISO-8601 ב-UTC אם ניתן."""
    raw = (job.get("updated_at") or job.get("created_at") or "").strip()
    if not raw:
        return ""
    try:
...
def scrape_riskified_jobs(keyword: str = "Data"):
    ...
    print(f"Found {len(jobs)} positions with '{keyword}' at Riskified")
    return jobs
...
if __name__ == "__main__":
    out_path = "riskified_jobs.csv"
    jobs = scrape_riskified_jobs(keyword="Data Scientist")
    upsert_and_mark_stale(out_path, jobs)
    print("Riskified positions were updated on ", out_path)
משרות
...
#     'Qualifications','Life at ','In the News',
#     # שדות ניהול:
#     'active','first_seen','last_seen','inactive_since'
# ]

# def utc_now_iso():
#     return datetime.now(timezone.utc).replace(microsecond=0).isoformat()

# # קנוניקליזציה פשוטה לקישורים — מסירים querystring וטראילינג סלאש כפול
# def canonicalize_link(url: str) -> str:
#     if not url:
#         return ""
#     u = url.strip()
#     # הורדת query/fragments
#     u = re.sub(r"[?#].*$", "", u)
#     # ניקוי סלאשים כפולים בסוף
#     u = re.sub(r"/+$", "/", u)
#     return u

# def normalize_row(job: dict) -> dict:
#     """ממלא שדות חסרים ומחזיר dict עם כל ה־BASE_FIELDS כמחרוזות."""
#     row = {}
#     for k in BASE_FIELDS:
#         v = job.get(k, "")
#         row[k] = "" if v is None else str(v)
#     # דואגים שה־link יהיה קנוני
#     row['link'] = canonicalize_link(row.get('link', ''))
#     return row

# def uniq_key(job: dict) -> str:
#     """
#     סדר עדיפויות למפתח ייחודי:
#     1) link (הכי יציב בין מקורות)
#     2) source:id
#     3) source:title
#     """
#     link = canonicalize_link(str(job.get('link') or "").strip())
#     if link:
#         return link
...
# # ---------------------------------------------
# # קריאה של ה־master אם קיים
# # ---------------------------------------------
# def load_master(path=MASTER_CSV) -> dict:
...
#                     r[k] = ""
#             # קנוניקליזציה לקישור
#             r['link'] = canonicalize_link(r.get('link', ''))
#             out[uniq_key(r)] = r
#     return out
...
# # שמירה של ה־master
# # ---------------------------------------------
# def save_master(rows: dict, path=MASTER_CSV):
#     fieldnames = BASE_FIELDS[:]  # סדר קבוע
#     with open(path, "w", newline="", encoding="utf-8-sig") as f:
#         w = csv.DictWriter(f, fieldnames=fieldnames)
#         w.writeheader()
...
# # ---------------------------------------------
# # כאן: שלבי הסקרייפרים שלך (ייבוא/קריאה)
# # ---------------------------------------------
# # דוגמה: את הייבוא השאירי כמו אצלך
# # from your_code import scrape_taboola_jobs, scrape_similarweb_jobs, scrape_melio_jobs
# # ודאי שכל סקרייפר מחזיר רשימת dict-ים עם השדות בסכמה (או חלקם),
# # ואנחנו ננרמל (normalize_row) בהמשך.

# def run_and_merge():
#     now = utc_now_iso()

#     # 1) טוענים master קיים
#     master = load_master()

#     # 2) מריצים את הסורקים שלך (החליפי לקריאות האמיתיות)
#     # דוגמה — נניח שהן קיימות בסקופ:
#     try:
#         taboola = scrape_taboola_jobs(keyword="Data Scientist")
#     except NameError:
#         taboola = []
#     try:
#         simweb  = scrape_similarweb_jobs(keyword="Data Scientist")
#     except NameError:
#         simweb = []
#     try:
#         melio   = scrape_melio_jobs(keyword="Data")
#     except NameError:
#         melio = []

#     # אפשר להוסיף כאן עוד מקורות בעתיד...

#     # 3) איסוף רשומות חדשות + מילוי scraped_at אם חסר
#     raw_rows = []
#     for lst in (taboola, simweb, melio):
#         for j in lst:
#             if not j.get('scraped_at'):
#                 j['scraped_at'] = now
#             raw_rows.append(j)

#     # דה־דופ מהיר לפי link לפני upsert (מונע כפילויות בין מקורות)
#     seen_links = set()
#     new_rows = []
#     for r in raw_rows:
#         r_norm = normalize_row(r)
#         lk = r_norm.get('link', '')
#         if lk and lk in seen_links:
#             continue
#         if lk:
#             seen_links.add(lk)
#         new_rows.append(r_norm)

#     # 4) סימון שלא נראו בריצה זו
#     for k in master:
#         master[k]['_seen_this_run'] = False
...
#     # 5) הוספה/עדכון לפי הריצה
#     for r in new_rows:
#         k = uniq_key(r)
#         if k in master:
#             # קיימת — לא מוחקים מידע, רק מעדכנים שדות חסרים/ניהוליים
#             seen_count += 1
#             master[k]['_seen_this_run'] = True

#             # אם הייתה לא פעילה וחזרה — נסמן הפעלה מחדש
#             if master[k].get('active') == "0":
#                 master[k]['active'] = "1"
#                 master[k]['inactive_since'] = ""
#                 reactivated_count += 1

#             # עדכון טכני
#             master[k]['last_seen'] = now
#             if not master[k].get('first_seen'):
#                 master[k]['first_seen'] = now

#             # אפשר לעדכן גם שדות "בטוחים" שלא אמורים לפגוע בנתונים קיימים
#             # לדוגמה: אם היה location ריק ומגיע חדש — נמלא
#             for field in ['location','date_posted','source','link',
#                           'About Us','About the Role',"What You'll Be Doing",
#                           'Qualifications','Life at Riskified','In the News']:
#                 if not master[k].get(field) and r.get(field):
#                     master[k][field] = r[field]
...
#             r['last_seen'] = now
#             r['inactive_since'] = ""
#             r['_seen_this_run'] = True
#             master[k] = r

#     # 6) מה שהיה בקובץ ולא הופיע הפעם — הפוך ללא פעיל (stale)
#     stale_count = 0
#     for k, row in master.items():
#         if not row.get('_seen_this_run'):
#             if row.get('active') != "0":
#                 row['active'] = "0"
...
#         master[k].pop('_seen_this_run', None)

#     # 7) שמירה
#     save_master(master)
#     total = len(master)
#     print(f"נשמרו {total} רשומות ב־{MASTER_CSV} | new={new_count}, seen={seen_count}, reactivated={reactivated_count}, stale={stale_count}")
